...
const sections = [
  ...
  },
  {
    ...
    title: "设计原则",
    items: [
      { icon: <PrincipleIcon type="restraint" />, title: "克制", desc: <><p className="font-medium text-[var(--neutral-900)]">色彩克制</p><p>品牌红仅作为品牌签名出现，产品蓝仅用于可交互元素，大面积使用白色和灰阶构建界面。</p><p className="mt-3 font-medium text-[var(--neutral-900)]">装饰克制</p><p>以信息传达为优先，减少无关视觉噪声。官网等品牌叙事层允许适度的视觉强化，装饰元素遵循统一数值梯度。</p></> },
      { icon: <PrincipleIcon type="clarity" />, title: "清晰", desc: <><p className="font-medium text-[var(--neutral-900)]">信息层级清晰</p><p>通过严格的字号、字重、灰度体系，确保用户快速建立信息浏览动线。</p><p className="mt-3 font-medium text-[var(--neutral-900)]">操作路径清晰</p><p>视觉权重递进明确，用户能无歧义地找到主行动点。</p></> },
      { icon: <PrincipleIcon type="trust" />, title: "可信", desc: <><p className="font-medium text-[var(--neutral-900)]">结构可信</p><p>通过稳定结构和明确边界，建立数据治理的可靠感。</p><p className="mt-3 font-medium text-[var(--neutral-900)]">反馈可信</p><p>状态反馈及时准确，权限标识清晰可辨，操作结果可预期。</p></> },
      { icon: <PrincipleIcon type="efficiency" />, title: "高效", desc: <><p className="font-medium text-[var(--neutral-900)]">操作高效</p><p>面向材料工程师、工艺师、数据运营方等专业用户，减少无效步骤。</p><p className="mt-3 font-medium text-[var(--neutral-900)]">信息密度高效</p><p>紧凑而有序地呈现高密度数据，减少不必要的留白和滚动。</p></> },
    ],
  },
] as const;
      ...
      <div className="relative z-10 mx-auto flex min-h-[450px] max-w-[calc(var(--content-docs-width)+12rem)] items-start gap-10 px-5 pt-20 md:px-8 xl:px-10">
        <div className="min-w-0 flex-1">
          <div className="mb-3 inline-flex items-center gap-2 rounded-sm border border-[var(--neutral-200)] bg-white px-3 py-1.5 text-xs font-medium text-[var(--neutral-600)]">
            <span className="text-[var(--neutral-500)] mr-1.5">2026</span><span className="h-1.5 w-1.5 rounded-full bg-[var(--brand-600)]" />
            新材道设计系统
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {sections[0].items.map((item) => (
            <SectionCard key={item.title} className="p-8">
              <div className="mb-3 flex h-8 w-8 items-center text-[var(--neutral-700)]">
                {item.icon}
              </div>
            ...
            <SectionCard key={item.title} className="p-8">
              <div className="flex items-start gap-10">
                <div className="shrink-0">
                  <div className="flex h-8 w-8 items-center text-[var(--neutral-700)]">
                    {item.icon}
                  </div>
                  <h3 className="mt-2 text-lg font-medium leading-[26px] text-[var(--neutral-900)] whitespace-nowrap">{item.title}</h3>
                </div>
                <div className="min-w-0 text-sm leading-[22px] text-[var(--neutral-600)]">{item.desc}</div>
              </div>
            </SectionCard>
          ))}
        </div>
