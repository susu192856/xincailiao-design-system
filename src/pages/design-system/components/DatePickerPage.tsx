    ...
    <section>
      ...
      <SectionCard className="overflow-hidden !p-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[360px_minmax(0,1fr)]">
          ...
          <div className="bg-[var(--neutral-50)] p-5 md:p-8"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h4 className="text-sm font-semibold text-[var(--text-primary)]">实时预览</h4><p className="mt-1 text-xs text-[var(--text-tertiary)]">可直接打开面板并操作。</p></div><div className="flex flex-wrap gap-1.5">{summary.map(item=><span key={item} className="rounded-full border border-[var(--neutral-200)] bg-white px-2.5 py-1 text-[11px] text-[var(--text-secondary)]">{item}</span>)}</div></div><div className="flex min-h-[420px] items-start justify-center overflow-x-auto rounded-[var(--radius-md)] border border-dashed border-[var(--neutral-300)] bg-white p-8"><div key={mode} className="w-full transition-all duration-[var(--motion-duration-slow)] ease-[var(--motion-easing-standard)] animate-scale-in"><div className="w-full max-w-[492px]">{preview}</div></div></div>{mode === "range" ? <div className="mt-4 animate-fade-in-slow rounded-[var(--radius-md)] border border-[var(--product-blue-200)] bg-[var(--info-bg)] px-4 py-3 text-xs leading-5 text-[var(--text-secondary)]"><strong className="text-[var(--text-primary)]">起止日期模式：</strong>预览区展示完整的范围选择流程——先选起始日期确认，焦点自动进入结束日期；再次确认后完成范围。</div> : mode === "dateTime" ? <div className="mt-4 animate-fade-in-slow rounded-[var(--radius-md)] border border-[var(--product-blue-200)] bg-[var(--info-bg)] px-4 py-3 text-xs leading-5 text-[var(--text-secondary)]"><strong className="text-[var(--text-primary)]">日期+时间模式：</strong>日历面板右侧显示时间选择列，选定日期后可调整小时和分钟。点"确认"关闭面板。</div> : mode === "time" ? <div className="mt-4 animate-fade-in-slow rounded-[var(--radius-md)] border border-[var(--product-blue-200)] bg-[var(--info-bg)] px-4 py-3 text-xs leading-5 text-[var(--text-secondary)]"><strong className="text-[var(--text-primary)]">独立时间模式：</strong>使用 TimePicker，不显示日历面板；下拉展示时间列表，步长为 15 分钟。仅在业务完全不依赖日期时使用。</div> : null}</div>
        </div>
      </SectionCard>
    </section>
  );
...
<DatePicker label="发布日期" defaultValue="2026-07-02" />`} />
      ...
      <section>
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 rounded-[var(--radius-md)] border border-[var(--neutral-200)] bg-white p-5 text-sm text-[var(--text-secondary)] md:grid-cols-4">
