  ...
  return (
    <div className="space-y-16">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-5">
          <ExampleCard title="Small · 28px" description="用于紧凑表格、窄工具栏和信息密度较高的后台页面。"><Pagination page={2} total={6} size="sm" /></ExampleCard>
          <ExampleCard title="Medium · 32px" description="常规列表、搜索结果和默认后台页面使用的标准尺寸。"><Pagination page={2} total={6} size="md" /></ExampleCard>
        </div>
      ...
      <section>
        <SectionHeading eyebrow="Composition" title="表格组合" description="分页作为表格 footer 与数据区共用一个外边框，并通过顶部分隔线区分；总数放在左侧，分页放在右侧。" />
        <div className="bg-white p-6">
          <div className="overflow-hidden rounded-[var(--radius-sm)] border border-[var(--neutral-200)] bg-white">
            <TableContainer className="rounded-none border-0"><Table density="compact" className="min-w-[640px]"><TableHeader><TableRow><TableHead className="w-10"><Checkbox aria-label="全选当前页" size="sm" /></TableHead><TableHead>材料名称</TableHead><TableHead>数据状态</TableHead><TableHead>更新时间</TableHead></TableRow></TableHeader><TableBody>{[["TC4 钛合金", "2026-06-04"], ["高温合金 GH4169", "2026-06-03"], ["铝合金 6061", "2026-06-02"]].map(([item, date]) => <TableRow key={item}><TableCell><Checkbox aria-label={`选择 ${item}`} size="sm" /></TableCell><TableCell>{item}</TableCell><TableCell><Tag variant="success" size="sm" dot>已治理</Tag></TableCell><TableCell className="tabular-nums">{date}</TableCell></TableRow>)}</TableBody></Table></TableContainer>
            <div className="flex flex-col gap-3 border-t border-[var(--neutral-200)] bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <section>
        ...
        <SpecList
          items={[
            "分页作为表格 footer 与数据区共用外边框；总数单行显示在左侧，分页放在右侧。",
