    ...
    <div className="space-y-16">
      ...
      <section>
        ...
        <ExampleCard title="数据资产列表" description="这是默认完整结构，不用于讲解所有边界状态；下方模块分别解释每一项规则。">
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-[var(--text-tertiary)]">共 128 条，当前展示 1–3 条</p><Pagination page={1} total={4} /></div>
        </ExampleCard>
      </section>
      ...
      <section>
        <SectionHeading eyebrow="Spacing" title="列间距" description="表格不单独使用 gap 控制列距；相邻列的内容间距由两个单元格的左右内边距共同产生，并随行高密度统一变化。" />
        <ExampleCard title="由单元格内边距形成列距" description="空间不足时调整列宽、精简字段或在 TableContainer 内启用横向滚动，不把相邻列内容压缩到 24px 以下。">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <div className="border-l-2 border-[var(--product-blue-300)] bg-[var(--neutral-50)] px-4 py-3 text-xs leading-5 text-[var(--text-secondary)]"><strong className="text-[var(--text-primary)]">设计规则：</strong>列宽负责分配内容空间，单元格内边距负责形成列距；不要在 table、tr 或单元格之间追加 gap。</div>
      ...
      <section>
        ...
        <div className="mt-5">
          <ExampleCard title="文本显示" description="默认单行显示，超出列宽后省略；只有描述类内容或阅读型表格允许两行或完整换行。省略内容必须能通过 Tooltip、详情或展开行查看全文。">
      ...
      <section>
        ...
        <ExampleCard title="展开、排序与更多操作" description="状态表头保持普通文字；排序支持升序、降序和取消排序，展开内容紧邻所属数据行。">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            ...
            <div className="border border-[var(--neutral-200)] bg-white p-4"><p className="text-sm font-semibold text-[var(--text-primary)]">2 个操作</p><div className="mt-3 flex gap-4"><Button variant="text" tone="product" size="sm" className="!h-auto !px-0 !py-0">编辑</Button><Button variant="text" tone="product" size="sm" className="!h-auto !px-0 !py-0">复制</Button></div><p className="mt-2 text-xs leading-5 text-[var(--text-secondary)]">保留两个短文字，顺序按频率排列。</p></div>
            <div className="border border-[var(--neutral-200)] bg-white p-4"><p className="text-sm font-semibold text-[var(--text-primary)]">3 个及以上</p><div className="mt-2 flex items-center gap-2"><Button variant="text" tone="product" size="sm" className="!h-auto !px-0 !py-0">查看</Button><Button variant="text" tone="neutral" size="sm" icon={<DotsThree size={18} />} aria-label="更多操作" title="更多操作" className="h-8 w-8 !px-0" /></div><p className="mt-2 text-xs leading-5 text-[var(--text-secondary)]">有明确主操作时保留一个文字入口；没有时仅显示“更多”。</p></div>
          </div>
          <p className="mt-3 border-l-2 border-[var(--neutral-300)] pl-3 text-xs leading-5 text-[var(--text-secondary)]"><strong className="text-[var(--text-primary)]">文字与图标：</strong>文字用于高频、业务语义不够通用或需要明确后果的操作；无文字图标仅用于“更多”等认知稳定的动作，并必须提供 Tooltip 与可访问名称。删除、停用等危险操作不得只用图标表达。</p>
        </ExampleCard>
      </section>
