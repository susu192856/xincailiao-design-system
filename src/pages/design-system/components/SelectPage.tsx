...
function SelectPlaygroundSection() {
  const [mode, setMode] = useState<"single" | "search" | "multiple">("single");
  ...
  const summary = [{single:"常规单选",search:"搜索式选择",multiple:"多选"}[mode], {sm:"Small · 28px",md:"Medium · 32px",lg:"Large · 36px"}[size], layout === "left" ? "左右标签" : "上下标签", {default:"默认",error:"错误",disabled:"禁用",loading:"加载"}[state]];
...
export default function SelectPage() {
  return (
    <div className="space-y-16">
      <PageHeader
        title="选择器"
        description="选择器用于从固定范围选值，覆盖单选、多选和搜索。与输入框共用字段结构、标签布局和状态 Token，但只接受预设选项，不承担自由文本录入。"
      ...
      <section>
        <SectionHeading eyebrow="Types" title="选择器类型" description="按值的数量选择组件模式，不用外观相似的控件互相替代。" />
        <SectionCard className="md:p-6"><div className="max-w-[492px] space-y-4"><Select labelPosition="left" labelWidth={96} label="常规选择" defaultValue="metal" options={materialOptions} /><Select labelPosition="left" labelWidth={96} label="多选" multiple defaultValue={["heat","aviation"]} options={tagOptions} /><Select labelPosition="left" labelWidth={96} label="搜索式选择" searchable placeholder="输入关键词搜索" options={searchableOptions} /></div></SectionCard>
...
<Select label="材料类型" options={options} />`} />
      ...
      <section>
        <SectionHeading eyebrow="Panels" title="展开面板与选项状态" description="面板直接展开比较；Hover 使用 neutral 底色，选中使用 neutral-100 底 + neutral-900 文字。每行至少 32px，最多完整显示 6 行。" />
