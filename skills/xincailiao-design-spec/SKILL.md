...
version: 0.2.0
description: 新材道设计规范 Skill。生成、修改或审查新材道官网、门户、数据空间、材库、材小模和后台系统的 UI 时使用。覆盖 32 个共享组件、7 个基础规范、桌面优先的移动适配、React/Vue/Figma 合同和页面模式。不用于纯后端逻辑、非新材道品牌项目或与视觉规范无关的任务。
---
...
视觉治理补充：修改规范站页面时同时阅读仓库根目录的 `docs/VISUAL_GOVERNANCE.md`。说明型表格使用 `DocsTable`，页面章节标题使用 `SectionHeading` 或 `SubsectionHeading`，不要在页面中临时写红色或蓝色标题色块。
