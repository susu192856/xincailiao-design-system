...
- Input / Textarea 桌面高密度配置场景优先使用左右标签；标签降为 14px Regular 与次级正文色，Hover / Focus 使用 neutral-900 单边框且不叠加额外黑色外描边。
- Textarea 增加 `labelPosition` / `labelWidth` 合同，并在移动端回退为上下标签。
- Textarea 规范并入输入框页面，补齐 Input / Textarea 的构成、尺寸、状态、结构、属性和使用边界，并移除独立文本域导航与路由。
