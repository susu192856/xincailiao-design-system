...
各产品线后台共享同一骨架（顶栏+侧栏+内容区），差异仅体现在：

| 产品线 | 主操作色 | 侧栏色 | 独有特征 |
|---|---|---|---|
| 新材道后台 | product-blue | 深色侧栏 | 通用管理 |
| 数据空间后台 | product-blue | 白色侧栏 | 数据目录 Tree 组件 |
| 材库后台 | product-blue | 白色侧栏 | 材料卡片网格视图 + 表格视图切换 |
| 材小模 | product-blue | 白色侧栏 | 对话式交互面板 |

**材库后台独有规则：**
- 列表页支持"卡片视图"和"表格视图"两种模式切换（Toggle 用 Tabs `variant="segment"`）
- 卡片视图每行 3-4 个材料卡片，卡片带缩略图（Image 组件 `variant="square"`）
- 材料分类树在侧栏下方（Tree 组件 `variant="directory"`）
- 材料状态 Tag 使用 soft 变体：上架=success、下架=neutral、审核中=warning
- 批量操作栏在表格上方：选择材料后出现，包含"批量上架/下架/移动分类"
