...
const components = [
  ...
  },
  {
    name: "Toast",
    title: "消息反馈",
    desc: "包含顶部轻提示、通知提示和页面提示，按影响范围与阅读时长选择形态。",
    props: [
      ["presentation", "'toast' | 'notification' | 'alert'", "'toast'", "展示形态"],
      ["tone", "'success' | 'error' | 'warning' | 'info' | 'loading'", "'info'", "语义类型"],
      ["title", "string", "—", "标题"],
      ["description", "string", "—", "描述"],
    ],
    guidelines: [
      { type: "do", text: "按顶部轻提示、通知提示和页面提示选择正确形态" },
      { type: "dont", text: "不要用消息反馈展示需要用户确认的信息" },
    ],
  },
  {
    name: "Button",
...
.variant-text:hover { background: var(--neutral-50); }
  ...
  },
  {
    ...
    props: [
      ...
      ["variant", "'default' | 'decision'", "'default'", "普通布局或居中的决策/语义布局"],
      ["title", "string", "—", "标题"],
    ...
    guidelines: [
      { type: "do", text: "基础对话框按任务区分带底部操作区与无底部操作区两种变体" },
      { type: "do", text: "普通对话框标题与说明保持简洁；决策和语义对话框使用居中图标与标题，不设置标题说明" },
      { type: "do", text: "footer 左侧放取消、右侧放确认，符合用户习惯" },
      { type: "dont", text: "不要在一个页面同时打开多个对话框" },
    ],
  },
  {
    name: "Table",
...
function getComponentDescription(name) {
  const map = {
    ...
    Modal: "用于承载需要用户确认或专注处理的任务，覆盖普通、危险、警告和成功反馈。默认宽 504px，详情型上限 960px。",
