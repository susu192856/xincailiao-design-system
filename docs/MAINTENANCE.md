...
- 首页只说明设计系统的目标、原则和适用产品，不承载文档编写约定。
- 所有页面先用面向普通浏览者的中文说明“是什么、何时使用、操作结果”；英文组件名、状态名和属性名紧邻中文出现，例如“悬停（Hover）”“描边型（outline）”。
- 设计师需要的结构、尺寸、状态、响应式和 Figma 映射放在对应组件或基础规范页，不要求先阅读代码。
- 开发专属的属性、接口（API）、设计变量代码名和最小代码示例放在明确的实现区；代码标识保留英文并使用等宽字体。
- 交付与维护页可以说明角色差异和同步流程，但不能用一段全局说明代替逐页内容治理。
- 修改页面文案后运行 `npm run verify:terminology`；校验会检查常见状态、组件和交付术语是否缺少中英文对应。
