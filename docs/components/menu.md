...
> 用于导航和功能入口组织，覆盖侧栏、顶部菜单、分组和折叠状态。

- 规范页面：`/components/menu`
- React 源码：`src/components/ui/Menu.tsx`
- Vue 源码：`packages/vue-ui/src/components/XcMenu.vue`
- Figma 组件名：`Menu`
...
**适用：** 用于Menu对应的标准场景，优先复用组件而不是在页面内重新绘制。
...
| `items` | `array` | `—` | 组件条目数据。 |
| `value` | `string` | `—` | 当前受控值。 |
| `orientation` | `unknown` | `—` | orientation 的组件合同字段；实现与文档必须保持一致。 |
| `collapsed` | `unknown` | `—` | collapsed 的组件合同字段；实现与文档必须保持一致。 |
...
```ts
import { XcMenu } from "@xincailiao/vue-ui";
